...
# Data
data<-read.table("tibiotarsusbirds1.txt", header=T)

# Tree
tree<-read.nexus("untitled2.trees.trees.trees.trees.nex")
...
data<-read.table("tibiotarsusbirds1.txt", header=T) # Reload data to plot it with log10 conversion
...
plot(NA,ylim=c(1L,length(tree2$tip.label)),xlim=lab,axes=FALSE,xlab="RMR (mLO2 h-1 g-0.93)")
lab <- lab[1L]:lab[2L]
...
# Filter out rows with NA in V0
valid_rows <- !is.na(data[,"V0"])

# Ensure that yfit corresponds to the filtered valid rows
# Assuming yfit needs to be filtered to match the data being plotted
filtered_yfit <- yfit[valid_rows]

# Match species with tree tips and create the y-values
y_values <- match(data[valid_rows, "species"], tree2$tip.label[ordered_tips])

# Create the x-values based on filtered yfit
x_values <- log10(exp(filtered_yfit))

# Ensure the lengths match before plotting
if (length(y_values) == length(x_values)) {
  points(y = y_values, x = x_values, pch = 4, cex = 2)
} else {
...
points(y=match(data[is.na(data[,"V0"]),"species"],tree2$tip.label[ordered_tips]),
       x=log10(exp(ypred$value)),pch=22,bg="orange",cex=2)
...
data[,c(2,4:6)]<-log(data[,c(2,4:6)]); data[,3]<-log1p(data[,3])
